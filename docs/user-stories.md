...
- A still-pending request's row (either tab) shows a truncated, quoted preview of its attached introduction, if one was sent (see ORP-003) — visible before deciding whether to accept, not hidden behind an extra click. An Established row never shows this preview; by then the message has already moved into the conversation thread itself.
- A still-pending request's row also shows the amount that was sent with it, but only when that amount is meaningfully above the routine fixed signal amount (i.e. a Direct Send-bundled request, not a plain "Add Contact" click) — a routine request always spends the same small fixed amount, so repeating that number on every row would just be noise.
...
- Right-clicking a `Message` bubble's text (mine only) opens a context menu with "Delete" and "Edit Message". Message text itself is a plain, non-interactive label — it's deliberately not left-clickable, so reading a message never pops up new buttons under the cursor. "Edit Message" opens a modal with the current text, a live character counter, and a Save button disabled while over the 1000-character limit — saving replaces the document in place, and the thread already renders an "(edited)" tag whenever a message's update time differs from its creation time. "Delete" opens a confirming modal ("Delete Message?") and, on confirmation, permanently removes the document — a true delete, not a placeholder, so the message disappears from both parties' conversation entirely, with no "this message was deleted" trace left behind. Deletion is offered only for `Message` content, never for a `Payment` or `PaymentRequest`. While an edit or delete for a bubble is in flight, that bubble shows its own progress label (e.g. "Deleting…") regardless of whether the context menu is open.
- A `Payment`'s memo (mine only) is editable the same way as a message's text — right-click to reveal "Edit Memo" when a memo already exists, or an always-visible "Add Memo" button when it doesn't. The amount is never editable, since it's sourced from the shielded transfer itself, not the message.
