...
handshake plus DPNS search (Milestone D), and messaging send/receive
(Milestone E: `Message`/`Payment`/`PaymentRequest`) have all landed and are
...
registration, HD-deriving `data`'s ENCRYPTION/DECRYPTION keys (decided, not
implemented). See `docs/ORCHARDPAY_MIGRATION.md` for how this relates to
...
### HD-deriving the ENCRYPTION/DECRYPTION identity keys (decided 2026-07-20, not yet implemented)
...
**Migration is not automatically clean, unlike `anchorData`.**
`generate_orchardpay_key` is live code — any identity that has already
published a `shieldedAddress` already had OrchardPay-bound
ENCRYPTION/DECRYPTION keys generated, and those are a **random**,
non-seed-derived keypair today. Switching `generate_orchardpay_key` to
seed-derivation only protects *newly* generated keys going forward;
already-registered random keys stay exactly as unrecoverable as before
unless rotated. Whether to rotate them (register a new HD-derived key,
disable the old one) is not yet decided — check the actual state of
already-registered OrchardPay keys on the target network before
implementing, since the answer changes if any real identities already carry
a random key.
...
- **Decided, not yet implemented (2026-07-20)**: HD-deriving `data`'s
  ENCRYPTION/DECRYPTION identity keys — reusing `identity_authentication_path`
  (no new DIP-13 sub-feature) with dynamic, gap-limit-scanned `key_index`
  assignment (no fixed reservation). See "HD-deriving the
  ENCRYPTION/DECRYPTION identity keys" above. Migration for
  already-registered random keys is still an open question.
