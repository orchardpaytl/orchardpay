...
log and `orchardpay-stderr.log` / `orchardpay.log` after a crash or panic.
...
OrchardPay publishes its AccessKit tree over AT-SPI2 when accessibility is
...
Add `ORCHARDPAY_ACCESSIBILITY=1` to the launch command, focus the window
