...
//! orchardpay_scan_incoming_anchor_memos` — see
...
/// Run one pass of the incoming-memo scan for `seed_hash`'s wallet, then try
/// every detected `contactAnchor` signal against every identity in
/// `qualified_identities`. Trying every locally-known identity (rather than
/// resolving which identity a wallet's memo is "for") is deliberate: only
/// the identity actually holding the matching contract-bounded DECRYPTION
/// key can ever decrypt a given anchor, so mismatched attempts fail cheaply
/// and are simply skipped — see `contact_anchor::handle_incoming_anchor_signal`.
pub async fn scan_for_incoming_anchors(
    ...
    let (found, next_start_index) = backend
        .orchardpay_scan_incoming_anchor_memos(sdk, &seed_hash, network, start_index)
        .await?;
    ...
    for anchor_document_id in found {
        for identity in &qualified_identities {
            ...
            .await
            {
                ...
                Ok(false) => {}
                Err(e) => {
                    // A wrong-identity decrypt attempt or a transient
                    // network error on one signal shouldn't abort the rest
                    // of the pass — log and move on.
                    tracing::debug!(
