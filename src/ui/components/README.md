...
| `LegacyRecoverySection` | `legacy_recovery_section.rs` | `Vec<RecoveryItem>` | Per-identity offer to restore keys stranded in the previous version's saved data (issue #889). Borrows a `RecoveryPlan`; lists the restorable items by role label plus the ones that cannot be restored with their reason, and reports the approved set when Restore is pressed. Renders nothing for an empty plan, so the offer retires itself. Backed by `ui/state/legacy_recovery.rs` (`LegacyRecoveryState`), which dispatches detection and the restore. This module also owns the feature's copy: `completion_message(restored)` for the success banner, `exclusion_explanation(reason)` for an item that cannot be restored, and `recovery_item_labels(items)`, which names items through the shared `ui/masternodes::role_label_and_tip` vocabulary so a key reads the same here as on the "Manage keys" list. |
