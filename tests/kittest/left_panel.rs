...
///
/// OrchardPay's nav rail carries two more always-visible entries than
/// upstream's (OrchardPay itself, plus DashPay kept visible instead of
/// hidden — see the `ORCHARDPAY-TODO(dashpay-legacy)` note in
/// `left_panel.rs`), so "Settings" — the last item — sits below the
/// scrollable nav list's visible viewport at the harness's default size and
/// is clipped out of the interactive area even though its accessibility
/// node still reports an (unclipped) logical position. `scroll_to_me()`
/// brings it into the visible/interactive region first, matching what a
/// real user scrolling the list would do.
#[test]
