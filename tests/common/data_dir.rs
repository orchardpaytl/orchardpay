...
//! `ORCHARDPAY_DATA_DIR` env var. Without isolation those tests open the real
//! `~/.config/OrchardPay` data dir, crash on a pre-existing
...
const DATA_DIR_ENV: &str = "ORCHARDPAY_DATA_DIR";

/// Serializes data-dir isolation across tests. `ORCHARDPAY_DATA_DIR` is
...
/// RAII guard restoring the prior `ORCHARDPAY_DATA_DIR` value on drop and
...
impl Drop for DataDirGuard {
    fn drop(&mut self) {
        ...
        // test touches ORCHARDPAY_DATA_DIR while this guard is alive.
        unsafe {
            match &self.prior {
...
/// Runs `f` with `ORCHARDPAY_DATA_DIR` pointed at a fresh temp directory, then
